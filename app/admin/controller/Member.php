 ...
 class Member extends Base{
     ...
     public function add()
     {
         if(request()->isPost()){
             ...
             $flag = Db::name('member')->save($param);
             if($flag){
     ...
     public function edit()
     {
         if(request()->isPost()){
             ...
             $flag = Db::name('member')->where(['id'=>$id])->update($param);
             if($flag){
     ...
     {
         $id = input('param.id');
         $flag = Db::name('member')->where(['id'=>$id])->delete();
         if($flag){
     ...
     public function state()
     {
         $id = input('param.id');
         $map = ['id'=>$id];
         $status = Db::name('member')->where($map)->value('status');
         if($status == 1){
             Db::name('member')->where($map)->update(['status'=>0]);
             return json(['code'=>1,'data'=>'','msg'=>'操作成功']);
         }else{
            Db::name('member')->where($map)->update(['status'=>1]);
            return json(['code'=>1,'data'=>'','msg'=>'操作成功']);
         }
     }
     //头像上传
